...
`/simulation` 与真实交易共用开仓逻辑：YES/NO 两腿价格与手续费合计严格低于 97¢、FOK 全额成交校验、单腿成交后的 FAK 平仓、冻结与冷却规则完全一致。SDK 因手续费产生的小份额差异允许成交；两种结算结果中较小的兑付扣除两腿最高总支出后仍须至少保留 `$0.001` 净利。唯一差别是模拟盘只使用 `PAPER_INITIAL_CAPITAL_USDT`，不连接钱包也不发送真实订单。
