...
- 因此同一机会若真实 SDK 会因手续费缩减导致较小腿的最坏结算净利不足 `$0.001` 而拒绝，模拟盘也会拒绝记录成交；CLOB 盘口 WebSocket 快照更新会保留已读取的费率元数据。
- 模拟盘仍不连接钱包、不签名、不发单；虚拟资金是与真实盘唯一的交易账户差异。
- 版本号提升为 `2.7.8`；待完成全量测试、提交和推送。

## 2026-08-14 v2.7.7 手续费份额差异按最坏结算净利放行，开仓价差恢复严格大于 3¢

- SDK 对 FOK 买单按手续费缩减两腿目标份额时，不再要求约 4% 的额外份额缓冲。现在用签名订单的实际份额与两腿 `max_spend` 判断：`min(YES份额, NO份额) - (YES最高支出 + NO最高支出) >= $0.001` 才提交。因此不要求两腿等份，但任一结算结果仍至少留下 $0.001 净利；不足则拒绝。
- 发现、模拟与真实开仓统一为价差严格大于 3¢：`MIN_SPREAD_TO_OPEN_CENTS=3.0`、默认 `SLIPPAGE_BUFFER_CENTS=3`、两腿价格及手续费严格低于 97¢。97¢ 恰好不下单。
- 价格上限计算在分级舍入后若恰好触及 97¢，会退回到可覆盖观察盘口且严格低于 97¢ 的最低分级上限，避免边界舍入把合规订单错误拦截。
- 版本号提升为 `2.7.7`；验证：`.venv/bin/python -m pytest -p no:cacheprovider tests -q`（119 passed）、`bash -n start.sh deploy.sh stop.sh`、`git diff --check`。

## 2026-08-13 真实交易页系统错误日志

- “最近成交”下新增“系统日志”区块，仅展示当前服务进程启动以来的错误：扫描失败、真实账户快照失败、自动交易失败、自动兑换失败和环境自动登录失败；正常扫描及成功日志不写入该区块。
- 系统日志按最新优先显示，包含北京时间、来源和错误信息，并纳入 `/api/live/dashboard` 的 5 秒刷新。服务重启会重新开始记录，不存储私钥、API key 或普通运行日志。

## 2026-08-13 首页自动真实交易横向单行布局

- 资金分配区内的“自动真实交易”改为独占该区块的一整行：标题、运行状态和启用/停止按钮在桌面端横向排列，不再上下堆叠；窄屏保持纵向布局，确保可用性。

## 2026-08-13 首页资金分配输入框按真实内容自适应

- 修正上轮 CSS `field-sizing` 在部分浏览器中与固定宽度叠加导致数值截断的问题。BTC / ETH / XRP / SOL 输入框现在在首屏、保存后回填和用户输入时，按实际文字像素宽度动态扩展，并保留 92px 最小宽度；移动端仍全宽显示。

## 2026-08-13 首页机会表与资金分配布局调整

- 真实交易首页“实时套利机会”表合并 YES / NO 两列为单一“交易对”列，展示同一市场问题，避免重复文案。
- “自动真实交易”状态与启用/停止开关移入“资金分配”区块；桌面端贴靠该区块右侧，移动端自动换行。
- BTC / ETH / XRP / SOL 的资金百分比输入框当时曾按内容收窄，后续改为真实内容自适应，避免数值被截断。

## 2026-08-13 v2.7.6 允许手续费造成的小份额差异，开仓收紧至 <96¢

- 修复 v2.7.4 的错误拦截：SDK 因真实 taker fee 将 10 份调整为例如 9.9 份是正常现象，系统不再要求两腿精确等份。提交前读取 SDK 签名订单的实际目标份额，并验证 `min(YES份额, NO份额) > YES最高总支出 + NO最高总支出`；只有这个保本关系不成立才拒绝提交。
- 真正成交确认也允许小份额差异；两腿均 `matched` 且最小份额仍覆盖两腿 `max_spend` 时视为已成交。故不会再因正常手续费缩量反复产生“SDK 已缩减目标份额”失败记录。
- 开仓规则统一为严格 `<96¢`（而非原 `<=97¢`）：默认 `SLIPPAGE_BUFFER_CENTS=4`，`FEE_BUFFER` 也必须包含在 96¢ 内；价差恰好 4¢ 仅观察，不下单。仓位档位同步为 4–4.5¢ 30%、4.5–5.3¢ 60%、大于 5.3¢ 100%。真实与模拟盘、页面状态、systemd 默认参数同步。
- 版本号提升为 `2.7.6`；新增 SDK 份额缩量仍可保本、严格 96¢ 阈值回归覆盖。

## 2026-08-13 v2.7.5 模拟盘与真实盘执行规则统一

- `PaperRunner` 不再把扫描出的平均价直接记为模拟成交。现在与真实开仓共用 97¢ 总成本、`FEE_BUFFER`、每腿价格上限计算；只有 YES/NO 在当前模拟盘口都能以相同份额完整 FOK 成交时，才记录模拟持仓。
- 若仅一腿能在价格上限内成交，模拟盘同样立即用该 token 的 bid 深度模拟 FAK 平仓；全平则释放模拟资金并进入冷却，无法全平则冻结交易对，不再继续模拟开仓。模拟 FOK 的实际盘口成本及 `FEE_BUFFER` 都会写入模拟持仓/收益。
- `/simulation` 仍不读取钱包、不发送真实订单，唯一资金来源是 `PAPER_INITIAL_CAPITAL_USDT`；README 已明确此边界。
- 版本号提升为 `2.7.5`；新增全额 FOK、单腿 FAK、部分平仓冻结和手续费模拟收益回归覆盖。
